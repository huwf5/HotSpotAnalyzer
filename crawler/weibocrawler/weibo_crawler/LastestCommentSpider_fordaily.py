...
def get_last_wid():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute("SELECT wid FROM posts ORDER BY wid DESC LIMIT 1")
...
def is_within_one_month(publish_date):
    """检查发布时间是否在最近一个月内。"""
...
def main(id=None):
    ...
    cookie = config_json["cookie"]
    child_max_page = config_json.get("child_max_page", 5)
    limit = config_json.get("limit", 2000)
    count = 0
        ...
        with conn.cursor() as cursor:
            ...
            while True:
                ...
                if last_wid is not None:
                    query = "SELECT wid,publish_time FROM posts WHERE wid < %s ORDER BY wid DESC LIMIT 20"
                    cursor.execute(query, (last_wid,))
                else:
                    query = "SELECT wid,publish_time FROM posts ORDER BY wid DESC LIMIT 20"
                    cursor.execute(query)

                posts = cursor.fetchall()
                # print(posts)
                posts = [post for post in posts if is_within_one_month(post['publish_time'])]
